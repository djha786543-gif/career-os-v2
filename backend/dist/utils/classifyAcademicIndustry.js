...
function classifyAcademicIndustry(job) {
    ...
    if (academicCompanyKw.some(k => company.includes(k)))
        return 'Academic';
    if (industryCompanyKw.some(k => company.includes(k)))
        return 'Industry';
    // ── 2. Job title check ────────────────────────────────────────────────────
    ...
    if (academicTitleKw.some(k => title.includes(k)))
        return 'Academic';
    if (industryTitleKw.some(k => title.includes(k)))
        return 'Industry';
    ...
    if (academicDescKw.some(k => text.includes(k)))
        return 'Academic';
    if (industryDescKw.some(k => text.includes(k)))
        return 'Industry';
