...
exports.poojaMatchScore = poojaMatchScore;
exports.rescoreAllActiveJobs = rescoreAllActiveJobs;
exports.purgeGarbageJobs = purgeGarbageJobs;
...
// ─── Hard Filter ─────────────────────────────────────────────────────────────
function passesHardFilter(title, sector) {
    const t = title.trim();
    if (t.length < 6)
        return false;
    if (GARBAGE_TITLE_RE.test(t))
        return false;
    if (/\d+\s+(cardiovascular|postdoc|molecular|research)\s+jobs?\b/i.test(t))
        return false;
    if (/\bjobs?\s+in\s+(north america|united states|usa|uk|europe|global)\b/i.test(t))
        return false;
    // Assistant Professor always passes
    if (/assistant\s+professor/i.test(t))
        return true;
    ...
    if (NOISE_DISCIPLINE_RE.test(t))
        return false;
    const tl = t.toLowerCase();
    // Block "assistant" unless it precedes "professor" (already caught above)
    if (tl.includes('assistant') && !tl.includes('assistant professor'))
        return false;
    if (HARD_FILTER_TERMS.some(term => tl.includes(term)))
        return false;
...
function poojaMatchScore(title, snippet, orgName, sector) {
...
async function rescoreAllActiveJobs() {
    try {
        const { rows } = await client_1.pool.query(`SELECT id, title, snippet, org_name, sector FROM monitor_jobs WHERE is_active = true`);
        let updated = 0;
        for (const row of rows) {
            const ms = poojaMatchScore(row.title, row.snippet || '', row.org_name, row.sector);
            await client_1.pool.query(`UPDATE monitor_jobs SET match_score = $1, high_suitability = $2 WHERE id = $3`, [ms, ms >= 50, row.id]);
            updated++;
        }
        ...
        console.error('[Monitor] Rescore failed:', err.message);
        return 0;
    }
}
...
async function purgeGarbageJobs() {
    try {
    ...
    `);
        ...
        if (n > 0)
            console.log(`[Monitor] Purged ${n} garbage job entries`);
        return n;
    }
    catch (err) {
        console.error('[Monitor] Garbage purge failed:', err.message);
        return 0;
    }
}
...
If no matching open positions are found, return: []`;
        const raw = await withTimeout((0, geminiClient_1.geminiGroundedSearch)(prompt, 2500), 30000, `webSearch for ${org.name}`);
        ...
        try {
            ...
            console.error(`[Monitor] JSON parse failed for ${org.name}:`, parseErr.message);
            return [];
        }
        ...
        return parsed
            ...
            .filter((j) => isRelevant(j.title, j.snippet))
            .filter((j) => j.location && isRelevantLocation(j.location))
            .filter((j) => passesHardFilter(j.title, org.sector))
            .map((j) => {
            ...
            .filter((j) => j.matchScore >= 20) // minimum bar
    ...
    try {
        ...
        for (const item of itemMatches.slice(0, 15)) {
            ...
            if (!isRelevant(title, desc))
                continue;
            if (!passesHardFilter(title, org.sector))
                continue;
...
async function runFullScan() {
    ...
    try {
       ...
       LIMIT 10`);
        ...
        const expired = await client_1.pool.query(`UPDATE monitor_jobs
       SET is_active = false
