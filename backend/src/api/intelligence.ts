...
function profile(req: Request): 'dj' | 'pj' {
...
Base this on the 2025-2026 job market. Be specific and actionable.`,
    ...
    const text: string = d?.content?.find((b: any) => b.type === 'text')?.text ?? '{}';
    ...
    if (!res.headersSent) return res.json({ trends: STATIC_TRENDS[p], source: 'static', profile: p });
  }
});
...
router.get('/market/heatmap', (req: Request, res: Response) => {
  const p = profile(req);
...
router.get('/study/plan', (req: Request, res: Response) => {
  ...
  let urgency: 'critical' | 'high' | 'moderate' | 'comfortable';
  if      (daysRemaining <= 14)  urgency = 'critical';
  else if (daysRemaining <= 30)  urgency = 'high';
  else if (daysRemaining <= 60)  urgency = 'moderate';
