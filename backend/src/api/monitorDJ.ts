...
// GET /api/monitor/dj/test-search?org=Goldman+Sachs — hits Serper /jobs for given org
router.get('/test-search', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const org = DJ_MONITOR_ORGS.find(o => o.name === orgName) || DJ_MONITOR_ORGS.find(o => o.name === 'Goldman Sachs') || DJ_MONITOR_ORGS[0]
    ...
    res.json({ org: org.name, query: org.searchQuery, type: 'jobs', status: resp.status, results: data })
  } catch (err: any) {
