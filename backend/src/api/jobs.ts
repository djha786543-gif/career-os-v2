...
import { getSearchProfile, countryNameToAdzunaCode, AdzunaCountry } from '../config/searchProfiles';
import { searchPoojaJobsViaWebSearch } from '../services/webSearchJobService';
...
function resolveRegion(country?: string, region?: string): string | undefined {
  ...
  if (country) {
    ...
    const code = countryNameToAdzunaCode[c];
    if (code) {
      // Re-use adzunaFetcher's region strings via a local map
      const ADZUNA_CODE_TO_REGION: Record<AdzunaCountry, string> = {
...
router.get('/', async (req, res) => {
  try {
    ...
    if (forceRefresh) {
      const lastUsed = getCache(frKey) as number | undefined;
      const now = Date.now();
    ...
    if (candidateId === 'pooja') {
        ...
        const INGEST_REGION_MAP: Record<string, string> = {
        ...
        if (!countryKey || countryKey === 'all') {
            ...
            const [usResult, ukResult] = await Promise.allSettled([
                ingestJobs('pooja', ['US'], resolvedTrack),
                ingestJobs('pooja', ['Europe'], resolvedTrack),
            ]);
            adzunaJobs = [
            ...
            try {
                adzunaJobs = await ingestJobs('pooja', [region], resolvedTrack);
            } catch (err) {
            ...
            const adzunaCode = countryNameToAdzunaCode[countryKey];
            if (adzunaCode) {
                const profileMap = getSearchProfile('pooja', q.track);
                const profile = profileMap[adzunaCode];
                if (profile) {
                    console.log(`[Jobs] Pooja: ${countryKey} → Adzuna(${adzunaCode})`);
                    try {
                        const raw = await fetchAdzunaJobs(adzunaCode, profile);
                        ...
                        if (raw.length < 5) {
                            ...
                            try {
                                const webJobs = await searchPoojaJobsViaWebSearch(countryName, q.track);
            ...
            const ADZUNA_PROXY: Record<string, string> = {
            ...
            let webJobs: Job[] = [];
            try {
                webJobs = await searchPoojaJobsViaWebSearch(countryName, q.track, forceRefresh);
            } catch (err) {
            ...
            if (webJobs.length > 0) {
                ...
                const proxyCode = ADZUNA_PROXY[countryKey] as AdzunaCountry | undefined;
                if (proxyCode) {
                    const profileMap = getSearchProfile('pooja', q.track);
                    ...
                    if (profile) {
                        ...
                        try {
                            adzunaJobs = await fetchAdzunaJobs(proxyCode, profile);
                        } catch (err) {
        ...
        const adzunaCode: AdzunaCountry | undefined = countryParam
            ? countryNameToAdzunaCode[countryParam]
            : undefined;
        const useDirectFetch = !!adzunaCode && !REGION_COUNTRIES.has(countryParam);

        if (useDirectFetch && adzunaCode) {
            const profileMap = getSearchProfile(candidate.id, resolvedTrack);
            const profile = profileMap[adzunaCode];
            if (profile) {
                try {
                    adzunaJobs = await fetchAdzunaJobs(adzunaCode, profile);
