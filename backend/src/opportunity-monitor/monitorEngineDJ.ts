...
async function scanViaWebSearchDJ(org: DJMonitorOrg): Promise<DJScannedJob[]> {
  ...
  try {
    ...
    const results: any[] = data.jobs || data.organic || []
    console.log(`[MonitorDJ] ${org.name}: Serper raw=${results.length} (jobs API)`)

    const jobs: DJScannedJob[] = []
    for (const r of results) {
      // /jobs returns: title, company, location, date, description, link, jobHighlights
