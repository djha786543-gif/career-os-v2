...
const ACADEMIA_KEYWORDS = [
  ...
  'institute', 'college', 'school of', 'dept', 'professor', 'faculty',
  'research fellow', 'tifr', 'ncbs', 'jnu', 'du ', 'niser',
];
...
function matchesRegionFilter(job: any, region: string | null): boolean {
  ...
  const loc = (job.location || '').toLowerCase();
  if (region === 'DE') return loc.includes('german') || loc.includes('berlin') || loc.includes('munich') || loc.includes('hamburg') || job.region === 'Germany';
...
function buildApiPath(sector: string, region: string | null): string {
  ...
  if (sector === 'academia') {
    ...
    params.set('track', 'Industry');
  } else if (sector === 'india') {
    params.set('country', 'india');
  } else if (sector === 'international') {
    // For specific regions, pass country param to get more targeted results
    ...
    // ALL: fetch both US and UK for broadest coverage (backend handles this with country=all)
  }
...
function applyPoojaFilter(jobs: any[], sector: string, region: string | null): any[] {
  ...
  if (sector === 'academia') {
    filtered = filtered.filter(isAcademiaJob);
  } else if (sector === 'industry') {
    filtered = filtered.filter(isIndustryJob);
  } else if (sector === 'india') {
    filtered = filtered.filter(isIndiaLocation);
  } else if (sector === 'international') {
    filtered = filtered.filter(j => !isIndiaLocation(j));
    if (region) filtered = filtered.filter(j => matchesRegionFilter(j, region));
  }

  // Sort: fitScore desc, cap at 10 for crispness
  return filtered
...
const JobCard = ({ job }: { job: any }) => {
  const categoryColor = job.category === 'ACADEMIA' ? '#818cf8' : '#34d399';
  const categoryLabel = job.category === 'ACADEMIA' ? '🎓 Academia' : '🏭 Industry';
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          {job.category && (
            <span style={{ fontSize: 9, fontWeight: 900, padding: '2px 7px', background: job.category === 'ACADEMIA' ? 'rgba(129,140,248,0.12)' : 'rgba(52,211,153,0.12)', color: categoryColor, borderRadius: 4, flexShrink: 0 }}>
              {categoryLabel}
            </span>
          )}
        </div>
...
export const OpportunityMonitor = () => {
  ...
  const fetchJobs = useCallback(async (sector: typeof activeSector, region: string | null) => {
    ...
    setError(null);
    try {
      const path = buildApiPath(sector, region);
      const data = await api.get(path);
      const rawJobs: any[] = Array.isArray(data?.jobs) ? data.jobs : [];
      setTotalFetched(rawJobs.length);
      const filtered = applyPoojaFilter(rawJobs, sector, region);
      setJobs(filtered);
    } catch (err: any) {
      console.error('[OpportunityMonitor] fetch error:', err);
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      // Non-fatal: cache clear failed, still attempt fetch
    }
    ...
    <div style={{ color: 'white', fontFamily: 'var(--font-sans, sans-serif)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button
          ...
          style={{
            padding: '10px 24px', background: (scanning || loading) ? '#334155' : '#22c55e',
            color: 'white', border: 'none', borderRadius: 8, cursor: (scanning || loading) ? 'default' : 'pointer',
            fontWeight: 900, fontSize: 13, transition: 'background 0.2s',
          }}
        >
      ...
      {/* Region Sub-filters (only for international) */}
      ...
      {(jobs.length > 0 || totalFetched > 0) && (
        <div style={{ fontSize: 11, color: '#64748b', marginBottom: 12, marginTop: activeSector !== 'international' ? 16 : 0 }}>
          Showing {jobs.length} curated matches from {totalFetched} fetched · {SECTOR_LABELS[activeSector]}
          {activeRegion ? ` · ${activeRegion}` : ''}
          {' '}· Pooja profile alignment applied
        </div>
      )}

      {/* Results */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: activeSector !== 'international' ? 0 : 4 }}>
        {loading ? (
          <div style={{ padding: '48px', textAlign: 'center', color: '#64748b', fontSize: 13 }}>
            Fetching opportunities...
          </div>
        ) : error ? (
          <div style={{ padding: '24px', background: 'rgba(244,63,94,0.08)', border: '1px solid rgba(244,63,94,0.2)', borderRadius: 10, color: '#f43f5e', fontSize: 13, textAlign: 'center' }}>
            {error}
          </div>
        ) : jobs.length > 0 ? (
