    ...
    <div className="glass" style={{ padding: '24px' }}>
      <header style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '12px', marginTop: '4px' }}>Target: {profile === 'dj' ? 'IAPP AIGP (Q3 2026)' : 'ASCP MB (May 2026)'}</p>
        </div>
      </header>
      ...
      {loading ? <div className="spinner" /> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {(data?.weeks || []).map((week: any, wIdx: number) => (
            <div key={wIdx} className="glass" style={{ padding: '16px', background: 'rgba(0,0,0,0.02)' }}>
